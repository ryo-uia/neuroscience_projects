"""
SpikeAgent-style entrypoint for the mixed SC2 pipeline.
...
BASE_PIPELINE_PATH = THIS_FILE.parent / "spikeinterface_sc2_mixed_pipeline.py"
...
def load_base_pipeline():
    spec = importlib.util.spec_from_file_location("mixed_base_pipeline", BASE_PIPELINE_PATH)
...
def print_effective_summary(base, cfg: dict, ignored_overrides: list[str], base_argv: list[str]) -> None:
    print("SpikeAgent example wrapper (mixed)")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SpikeAgent example wrapper for mixed SC2 pipeline")
    parser.add_argument(
        "--job-config",
        type=Path,
        required=True,
